...
public class ASN1 {
        ...
        while (sequenceLength > 0) {
            ...
            if ((tag & 0x20) != 0) {
                ...
                if (tag == 6) {
                    ...
                    result.add(
                        new TagValue(tag, Arrays.copyOfRange(encoding, offset + l[0] + 1, offset + 1 + l[0] + l[1]))
                    );
                }
                offset += 1 + l[0] + l[1];
                sequenceLength -= 1 + l[0] + l[1];
    ...
    public static TagValue DecodeSimple(int offset, byte[] encoding) throws CoseException {
        ...
        if (encoding[offset] != 0x04) throw new CoseException("Invalid structure");
        int[] l = DecodeLength(offset + 1, encoding);

        int sequenceLength = l[1];
        if (offset + 2 + sequenceLength != encoding.length) throw new CoseException("Invalid sequence");
    ...
    public static ArrayList<TagValue> DecodePKCS8EC(ArrayList<TagValue> pkcs8) throws CoseException {
        ...
        if (pkdl.size() > 2) {
            if ((pkdl.get(2).tag & 0xff) != 0xA0) {
                ...
                if (pkdl.size() == 4 && (pkdl.get(3).tag & 0xff) != 0xa1) throw new CoseException(
