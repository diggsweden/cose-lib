...
public class COSEKey {
    ...
    public COSEKey(PublicKey pubKey, PrivateKey privKey) throws CoseException {
        ...
        if (pubKey != null) {
            ...
            if (Arrays.equals(alg.get(0).value, ASN1.oid_ecPublicKey)) {
                ...
                else if (Arrays.equals(oid, ASN1.Oid_secp384r1)) keyMap.Add(
                    KeyKeys.EC2_Curve.AsCBOR(),
                    KeyKeys.EC2_P384
                );
                else if (Arrays.equals(oid, ASN1.Oid_secp521r1)) keyMap.Add(
                    KeyKeys.EC2_Curve.AsCBOR(),
                    KeyKeys.EC2_P521
                );
                ...
                if (Arrays.equals(oid, ASN1.Oid_X25519)) keyMap.Add(KeyKeys.OKP_Curve.AsCBOR(), KeyKeys.OKP_X25519);
                else if (Arrays.equals(oid, ASN1.Oid_X448)) keyMap.Add(KeyKeys.OKP_Curve.AsCBOR(), KeyKeys.OKP_X448);
                else if (Arrays.equals(oid, ASN1.Oid_Ed25519)) keyMap.Add(
                    KeyKeys.OKP_Curve.AsCBOR(),
                    KeyKeys.OKP_Ed25519
                );
                else if (Arrays.equals(oid, ASN1.Oid_Ed448)) keyMap.Add(KeyKeys.OKP_Curve.AsCBOR(), KeyKeys.OKP_Ed448);
                else throw new CoseException("Unsupported curve");
        ...
        if (privKey != null) {
            ...
            if (Arrays.equals(alg.get(0).value, ASN1.oid_ecPublicKey)) {
                ...
                if (!keyMap.ContainsKey(KeyKeys.KeyType.AsCBOR())) {
                    keyMap.Add(KeyKeys.KeyType.AsCBOR(), KeyKeys.KeyType_EC2);
                    if (Arrays.equals(oid, ASN1.Oid_secp256r1)) keyMap.Add(
                        KeyKeys.EC2_Curve.AsCBOR(),
                        KeyKeys.EC2_P256
                    );
                    else if (Arrays.equals(oid, ASN1.Oid_secp384r1)) keyMap.Add(
                ...
                if (!keyMap.ContainsKey(KeyKeys.KeyType.AsCBOR())) {
                    keyMap.Add(KeyKeys.Algorithm.AsCBOR(), AlgorithmID.EDDSA.AsCBOR());
                    if (Arrays.equals(oid, ASN1.Oid_X25519)) keyMap.Add(KeyKeys.OKP_Curve.AsCBOR(), KeyKeys.OKP_X25519);
                    else if (Arrays.equals(oid, ASN1.Oid_X448)) keyMap.Add(
                        KeyKeys.OKP_Curve.AsCBOR(),
                        KeyKeys.OKP_X448
                    );
                    else if (Arrays.equals(oid, ASN1.Oid_Ed25519)) keyMap.Add(
                    ...
                    else if (Arrays.equals(oid, ASN1.Oid_Ed448)) keyMap.Add(
                        KeyKeys.OKP_Curve.AsCBOR(),
                        KeyKeys.OKP_Ed448
                    );
                    else throw new CoseException("Unsupported curve");
                ...
                ArrayList<ASN1.TagValue> pkdl = ASN1.DecodePKCS8EC(pkl);
                if (pkdl.get(0).tag != 4) throw new CoseException("Invalid PKCS8 structure");
    ...
    public CBORObject get(CBORObject keyValue) throws CoseException {
        if (
            (keyValue.getType() != CBORType.Integer) && (keyValue.getType() != CBORType.TextString)
    ...
    private void CheckKeyState() throws CoseException {
        ...
        if ((val == null) || (val.getType() != CBORType.Integer)) throw new CoseException(
            "Missing or incorrect key type field"
        );

        if (val.equals(KeyKeys.KeyType_Octet)) {
            val = get(KeyKeys.Octet_K);
            if ((val == null) || (val.getType() != CBORType.ByteString)) throw new CoseException(
    ...
    private void CheckECKey() throws CoseException {
        ...
        try {
            ...
            if (val == null) {
                ...
                (val.getType() != CBORType.ByteString) && (val.getType() != CBORType.Boolean)
    ...
    public ECGenParameterSpec GetCurve2() throws CoseException {
        if (get(KeyKeys.KeyType) != KeyKeys.KeyType_EC2) throw new CoseException("Not an EC2 key");
    ...
    public static COSEKey generateKey(AlgorithmID algorithm) throws CoseException {
     ...
     */
    public static COSEKey generateKey(AlgorithmID algorithm, String parameters) throws CoseException {
     ...
     * @throws CoseException
     */
    public static COSEKey generateKey(AlgorithmID algorithm, String parameters, Provider provider)
        throws CoseException {
        COSEKey returnThis;
        switch (algorithm) {
            case ECDSA_256:
    ...
    public static COSEKey generateKey(CBORObject curve, Provider provider) throws CoseException {
        String curveName;
        COSEKey returnThis;
    ...
    private static COSEKey generateECDHKey(String curveName, CBORObject curve, Provider provider) throws CoseException {
        try {
            int curveSize;
    ...
    private static COSEKey generateECDSAKey(String curveName, CBORObject curve, Provider provider)
        throws CoseException {
        try {
            int curveSize;
    ...
    private static COSEKey generateOkpKey(String curveName, CBORObject curve, Provider provider) throws CoseException {
        try {
            switch (curveName) {
                case "Ed25519":
                    break;
        ...
        if (n != null && e != null && d != null && p != null && q != null && dP != null && dQ != null && qInv != null) {
            ...
            if (other == null) {
                ...
                for (int i = 0; i < other.size(); i++) {
                    ...
                    if (
                        ri.getType() != CBORType.ByteString ||
    ...
    private static COSEKey generateRSAKey(int keySize, Provider provider) throws CoseException {
        try {
            KeyPairGenerator gen = provider != null
            ...
            key.add(KeyKeys.RSA_N, CBORObject.FromByteArray(priv.getModulus().toByteArray()));
            key.add(KeyKeys.RSA_E, CBORObject.FromByteArray(priv.getPublicExponent().toByteArray()));
            key.add(KeyKeys.RSA_D, CBORObject.FromByteArray(priv.getPrivateExponent().toByteArray()));
            key.add(KeyKeys.RSA_P, CBORObject.FromByteArray(priv.getPrimeP().toByteArray()));
            key.add(KeyKeys.RSA_Q, CBORObject.FromByteArray(priv.getPrimeQ().toByteArray()));
            key.add(KeyKeys.RSA_DP, CBORObject.FromByteArray(priv.getPrimeExponentP().toByteArray()));
            key.add(KeyKeys.RSA_DQ, CBORObject.FromByteArray(priv.getPrimeExponentQ().toByteArray()));
            key.add(KeyKeys.RSA_QI, CBORObject.FromByteArray(priv.getCrtCoefficient().toByteArray()));
