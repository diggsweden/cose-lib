...
public abstract class COSEObject extends Attribute {
     ...
     */
    public static COSEObject DecodeFromBytes(byte[] rgbData) throws CoseException {
     ...
     * @throws CoseException on a decode failure.
     */
    public static COSEObject DecodeFromBytes(byte[] rgbData, COSEObjectTag defaultTag) throws CoseException {
        CBORObject messageObject = CBORObject.DecodeFromBytes(rgbData);

        if (messageObject.getType() != CBORType.Array) throw new CoseException(
        ...
        if (messageObject.isTagged()) {
            ...
            if (defaultTag == COSEObjectTag.Unknown) {
                defaultTag = COSEObjectTag.FromInt(messageObject.getMostInnerTag().ToInt32Unchecked());
            } else if (defaultTag != COSEObjectTag.FromInt(messageObject.getMostInnerTag().ToInt32Unchecked())) {
        ...
        switch (defaultTag) {
            case Unknown: // Unknown
                throw new CoseException("COSEObject was not tagged and no default tagging option given");
            case Encrypt:
            ...
            case Sign:
                ...
                break;
            default:
                throw new CoseException("COSEObject is not recognized as a COSE security Object");
        }
