...
public abstract class SignCommon extends COSEObject {
    ...
    byte[] computeSignature(byte[] rgbToBeSigned, COSEKey cnKey) throws CoseException {
        AlgorithmID alg = AlgorithmID.FromCBOR(findAttribute(HeaderKeys.Algorithm));
        return computeSignature(alg, rgbToBeSigned, cnKey);
    }

    static byte[] computeSignature(AlgorithmID alg, byte[] rgbToBeSigned, COSEKey cnKey) throws CoseException {
        String algName = null;
        int sigLen = 0;
    ...
    boolean validateSignature(byte[] rgbToBeSigned, byte[] rgbSignature, COSEKey cnKey) throws CoseException {
        AlgorithmID alg = AlgorithmID.FromCBOR(findAttribute(HeaderKeys.Algorithm));
        return validateSignature(alg, rgbToBeSigned, rgbSignature, cnKey);
    }

    static boolean validateSignature(AlgorithmID alg, byte[] rgbToBeSigned, byte[] rgbSignature, COSEKey cnKey)
