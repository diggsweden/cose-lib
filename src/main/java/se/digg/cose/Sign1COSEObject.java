...
/**
 * The Sign1COSEObject class implements the COSE_Sign1 object.
 * This provides for a single signature where the signer is normally implicitly known.
 * {@link SignCOSEObject} need to be used in those cases where more than one signature is needed on a content.
 * There is no way to convert a signed message between the two formats.
 * <p>
 * Create a Sign1COSEObject object for a new message, when processing an existing message use COSEObject.DecodeFromBytes to create the Sign1COSEObject object.
 * <p>
...
public class Sign1COSEObject extends SignCommon {
    ...
    protected void DecodeFromCBORObject(CBORObject messageObject) throws CoseException {
        ...
        // Experimental attempt to detect CBOR Tagged data, and in such case, preserve the CBOR tags in the signed data.
        if (messageObject.get(2).HasTag(EInteger.FromInt32(24))) {
            rgbContent = messageObject.get(2).EncodeToBytes();
        } else {
            if (messageObject.get(2).getType() == CBORType.ByteString) rgbContent = messageObject
                .get(2)
                .GetByteString();
