...
public class Recipient extends COSEObject {
    ...
    public void DecodeFromCBORObject(CBORObject objRecipient) throws CoseException {
        if ((objRecipient.size() != 3) && (objRecipient.size() != 4)) throw new CoseException(
    ...
    public byte[] decrypt(AlgorithmID algCEK, Recipient recip) throws CoseException {
        ...
        switch (alg) {
            case Direct: // Direct
                if (privateKey.get(KeyKeys.KeyType.AsCBOR()) != KeyKeys.KeyType_Octet) throw new CoseException(
                    "Mismatch of algorithm and key"
                );
                return privateKey.get(KeyKeys.Octet_K.AsCBOR()).GetByteString();
            case HKDF_HMAC_SHA_256:
                if (privateKey.get(KeyKeys.KeyType.AsCBOR()) != KeyKeys.KeyType_Octet) throw new CoseException(
                    "Needs to be an octet key"
                );
                return HKDF(
            ...
            case ECDH_SS_HKDF_256:
                if (privateKey.get(KeyKeys.KeyType.AsCBOR()) != KeyKeys.KeyType_EC2) throw new CoseException(
                    "Key and algorithm do not agree"
                );
    ...
    public byte[] getKey(AlgorithmID algCEK) throws CoseException {
        ...
        switch (alg) {
            ...
            case AES_KW_192:
            case AES_KW_256:
                if (!privateKey.HasKeyType(KeyKeys.KeyType_Octet)) throw new CoseException(
                    "Key and algorithm do not agree"
                );
