...
public class RegressionTest extends TestBase {
    ...
    public void _VerifyEncrypt(CBORObject control, byte[] rgbData) throws CoseException {
        ...
        if ((cnFail != null) && (cnFail.getType() == CBORType.Boolean) && cnFail.AsBoolean()) {
            fFailBody = true;
        }
    ...
    public void _VerifyMac0(CBORObject control, byte[] rgbData) throws CoseException {
        ...
        try {
            ...
            if ((pFail != null) && (pFail.getType() == CBORType.Boolean) && pFail.AsBoolean()) {
                fFailBody = true;
            }
        ...
        for (CBORObject attr : cnAttributes.getKeys()) {
            switch (attr.AsString()) {
                ...
                case "kid":
                    ...
                    cnValue = CBORObject.FromByteArray(cnAttributes.get(attr).AsString().getBytes());
                    break;
                case "spk_kid":
                ...
                case "IV_hex":
                    ...
                    cnValue = CBORObject.FromByteArray(hexStringToByteArray(cnAttributes.get(attr).AsString()));
                    break;
                case "partialIV_hex":
    ...
    public COSEKey BuildKey(CBORObject keyIn, boolean fPublicKey) throws CoseException {
            ...
            switch (key.AsString()) {
                ...
                case "x":
                    cnKeyOut.set(
                        ...
                        CBORObject.FromByteArray(Base64.getUrlDecoder().decode(cnValue.AsString()))
                    );
                    break;
                case "x_hex":
                    cnKeyOut.set(
                ...
                case "kid":
                    cnKeyOut.set(
                        ...
                        CBORObject.FromByteArray(StandardCharsets.UTF_8.encode(cnValue.AsString()).array())
                    );
                    break;
                case "kid_hex":
                    cnKeyOut.set(
    ...
    int _ValidateSigned(CBORObject cnControl, byte[] pbEncoded) throws CoseException {
        ...
        try {
            ...
            for (iSigner = 0; iSigner < cnSigners.size(); iSigner++) {
                try {
                    COSEObject msg = COSEObject.DecodeFromBytes(pbEncoded, COSEObjectTag.Sign);
    ...
    int _ValidateSign0(CBORObject cnControl, byte[] pbEncoded) throws CoseException {
        ...
        try {
            ...
            try {
                COSEObject msg = COSEObject.DecodeFromBytes(pbEncoded, COSEObjectTag.Sign1);
    ...
    void AddCounterSignature0(COSEObject msg, CBORObject cSigInfo) throws CoseException, Exception {
    ...
    void AddCounterSignature(COSEObject msg, CBORObject cSigInfo) throws CoseException, Exception {
        if (
            (cSigInfo.getType() != CBORType.Map) ||
            !cSigInfo.ContainsKey("signers") ||
            (cSigInfo.get("signers").getType() != CBORType.Array)
        ) {
            throw new CoseException("invalid input file");
    ...
    void CheckCounterSignatures(COSEObject msg, CBORObject cSigInfo) {
        try {
            ...
            if (
                (cSigConfig.getValues().size() > 1) && (cSigs.getValues().size() != msg.getCountersignerList().size())
            ) {
                CFails++;
                return;
            }
